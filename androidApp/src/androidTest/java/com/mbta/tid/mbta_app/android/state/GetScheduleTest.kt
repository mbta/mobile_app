...
class GetScheduleTest {
    ...
    fun testSchedule() {
        ...
        composeTestRule.setContent {
            actualSchedules =
                getSchedule(stopIds = stopIds, ErrorKey(setOf(), "errorKey"), schedulesRepo)
        }
    ...
    fun testScheduleNoStops() {
        ...
        composeTestRule.setContent {
            actualSchedules =
                getSchedule(stopIds = emptyList(), ErrorKey(setOf(), "errorKey"), schedulesRepo)
        }
        ...
        composeTestRule.waitUntilDefaultTimeout {
            when (val errorState = errorRepo.state.value) {
                is ErrorBannerState.DataError ->
                    errorState.messages == setOf(ErrorKey(setOf(), "errorKey"))
                else -> false
            }
        }
    }
    ...
    fun testStopsBecomingNull() {
        ...
        composeTestRule.setContent {
            actualSchedules = getSchedule(stopIds, ErrorKey(setOf(), "errorKey"), schedulesRepo)
        }
