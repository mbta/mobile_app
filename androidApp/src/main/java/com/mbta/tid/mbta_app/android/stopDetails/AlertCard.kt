...
fun AlertCardPreview() {
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        AlertCard(
            ObjectCollectionBuilder.Single.alert({
                header =
                    "Providence/Stoughton Line Train S871 (6:22 PM from South Station) is cancelled today due to the holiday."
                effect = Alert.Effect.Cancellation
            }),
            AlertSummary(
                effect = Alert.Effect.Cancellation,
                location = AlertSummary.Location.FromCurrentStop("Mansfield"),
                timeframe =
                    AlertSummary.Timeframe.TripTime(
                        EasternTimeInstant(2026, Month.FEBRUARY, 26, 22, 17)
                    ),
            ),
            AlertCardSpec.Major,
            routeAccents =
