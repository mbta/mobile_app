...
struct HomeMapView: UIViewControllerRepresentable {
    ...
    func updateUIViewController(_ uiViewController: MapViewController, context: Context) {
        print("UPDATING")
        if ( uiViewController.mapView != nil) {
            print("HELLO")
            var vehicleManager = uiViewController.mapView.annotations.annotationManagersById["vehicles"] as! PointAnnotationManager
            vehicleManager.annotations = mapVM.vehicles.map { vehicle in
                var annotation = PointAnnotation(id: vehicle.id, coordinate: vehicle.coordinate)
                annotation.image  = .init(image: UIImage(named: "bus-icon")!, name: "bus-icon")
                return annotation
            }
        }
...
final class MapViewController: UIViewController {
    ...
    override func viewDidLoad() {
       ...
       CLLocationCoordinate2D(latitude: 42.347348, longitude: -71.068556),
       ...
       let lineAnnotations: [PolylineAnnotation] = routeFeatures.values.map { route in
           var lineAnnotation = PolylineAnnotation(lineCoordinates: route.coordinates)
           lineAnnotation.lineColor = StyleColor(route.color ?? .black)
       ...
       let lineAnnotationManager = mapView.annotations.makePolylineAnnotationManager()
        ...
        // used in updateUIViewController to set vehicle annotations
        mapView.annotations.makePointAnnotationManager(id: "vehicles")
    ...
    // From tutorial https://docs.mapbox.com/ios/maps/examples/change-icon-size/
    // which also shows how to add interactivity to layer
    private func addStopLayer() {
        let stopFeatures = stopsData.values.map{ stop in
            Feature(geometry: Point(stop.coordinate))
        }

        // Create a GeoJSON data source for markers
        var stopSource = GeoJSONSource(id: "stop-source")
        stopSource.data = .featureCollection(FeatureCollection(features: stopFeatures))
        try? mapView.mapboxMap.addSource(stopSource)

        // Add marker image to the map
        try? mapView.mapboxMap.addImage(UIImage(named: "t-logo")!, id: "t-logo")

        // Create a symbol layer for markers
        var stopLayer = SymbolLayer(id: "stop-layer", source: "stop-source")
        stopLayer.iconImage = .constant(.name("t-logo"))
        stopLayer.iconAllowOverlap = .constant(true)
        stopLayer.minZoom = 15


        try? mapView.mapboxMap.addLayer(stopLayer, layerPosition: .below("vehicles"))
