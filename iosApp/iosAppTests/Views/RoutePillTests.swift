...
final class RoutePillTests: XCTestCase {
    ...
    @MainActor func testShuttle() throws {
        ...
        XCTAssertEqual(
            try rlShuttleFixed.inspect().view(RoutePill.self).implicitAnyView().implicitAnyView().image().actualImage()
                .name(),
            "mode-bus"
        )
        XCTAssertEqual(
            try rlShuttleFlex.inspect().view(RoutePill.self).implicitAnyView().implicitAnyView().text().string(),
            "Red Line Shuttle"
        )
        XCTAssertEqual(
            try glShuttleFixed.inspect().view(RoutePill.self).implicitAnyView().implicitAnyView().image().actualImage()
                .name(),
            "mode-bus"
        )
        XCTAssertEqual(
            try glShuttleFlex.inspect().view(RoutePill.self).implicitAnyView().implicitAnyView().text().string(),
            "Green Line D Shuttle"
        )
